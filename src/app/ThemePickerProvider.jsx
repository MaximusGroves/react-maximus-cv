...
class ThemePickerProvider extends React.PureComponent {
  constructor (props) {
    ...
    this.allThemes = [
      {
        name: 'School Spirit - Light Theme',
        shortName: 'schoolSpirit',
        content: schoolSpirit
      },
      {
        name: 'Night Game - Dark Theme',
        shortName: 'nightGame',
        content: nightGame
      },
      {
        name: 'Edge Lord - Villain Theme',
        shortName: 'edgeLord',
        content: edgeLord
      }
    ];

    const cookieTheme = this.allThemes.find(
      theme => theme.shortName === this.props.cookies.get('theme')
    );
